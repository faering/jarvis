...
version `v`, and one `$defs` entry per message type ([ADR 0007](adr/0007-protocol-json-schema.md)).
...
add a message type, change the schema and both sides in one PR. Bump `v` only for a breaking change.
